...
class Mysql
{
    ...
    public function __construct($flag=0)
    {
        ...
        $this->connect($config['db'][$flag]["hostname"], $config['db'][$flag]["username"], $config['db'][$flag]["password"], $config['db'][$flag]["database"], $config['db'][$flag]["pconnect"]);
        $this->is_log = $config['db'][$flag]["log"];
        if ($this->is_log) {
            $handle = fopen($config['db'][$flag]["logfilepath"], "a+");
            $this->handle = $handle;
        }
